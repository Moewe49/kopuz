...
playlists = Playlists
playlist = Playlist
playlist_create_failed = Impossible de créer la playlist : { $error }
playlist_create_unsupported = Ce titre ne peut pas être enregistré sur le serveur actif.
album = Album
artist = Artiste
home = Accueil 
search = Chercher
...
discover_requires_ytmusic = Découvrir n'est disponible que lorsque YouTube Music est le serveur actif.
...
# Toast
added_to_playlist_toast = Ajouté à « { $name } »

# YT auto-refresh
yt_auto_refresh_label = Rester connecté automatiquement — relit les cookies d'un navigateur connecté au démarrage (gardez YouTube ouvert dans Firefox ; plus besoin de F12).
...
discord_test = Tester la connexion Discord
discord_test_button = Envoyer un test
download_all_offline = Tout télécharger pour la lecture hors ligne
download_folder = Dossier de téléchargement
onboarding_title = Bienvenue dans Kopuz
onboarding_subtitle = Deux façons de le remplir de musique :
onboarding_local_title = Dossier de musique local
onboarding_local_desc = Indiquez à Kopuz un dossier de fichiers audio pour les parcourir et les écouter.
onboarding_stream_title = YouTube Music ou un serveur
onboarding_stream_desc = Connectez YouTube Music (ou Jellyfin / Subsonic) pour écouter et télécharger.
onboarding_open_settings = Ouvrir les paramètres
onboarding_later = Plus tard
premium_downloads = Téléchargements premium (256 kbps)
premium_downloads_hint = Se connecte pour récupérer de l'AAC 256 kbps. Utile uniquement avec un compte YouTube Premium ; peut rarement déclencher une vérification anti-robot en cas de forte charge. Désactivé = 128 kbps anonyme.
remove_downloads = Supprimer les téléchargements
reset_to_default = Réinitialiser
search_elsewhere = Rien sur { $source } — d'autres sources proposent :
