language-name = Deutsch
is_rtl = false

settings = Einstellungen
theme_editor = Theme-Editor
appearance = Erscheinungsbild
language = Sprache
library = Bibliothek
playlists = Playlists
album = Album
artist = Künstler
home = Start
search = Suchen
favorites = Favoriten
radio = Radio
discover = Entdecken
logs = Protokolle
activity = Aktivität
general = Allgemein
music_directory = Musikordner
service = Dienst: { $name }
connected = ● Verbunden
disconnected = ● Getrennt
reconnect = Neu verbinden
remove = Entfernen
add = Hinzufügen
add_folder = Ordner hinzufügen
no_music_folders = Keine Musikordner konfiguriert
add_server = Server hinzufügen
connect_to_lastfm = Mit Last.fm verbinden
lastfm_connected = Mit Last.fm verbunden
server_details = Serverdetails
server_name = Servername
server_url = Serveradresse
choose_service = Dienst auswählen
cancel = Abbrechen
login = Anmelden
logging_in = Du wirst angemeldet...
login_to_service = Melde dich bei { $service } an
username = Benutzername
password = Passwort
player_settings = Player-Einstellungen
onboarding_title = Willkommen bei Kopuz
onboarding_subtitle = Zwei Wege, um es mit Musik zu füllen:
onboarding_local_title = Lokaler Musikordner
onboarding_local_desc = Zeige Kopuz auf einen Ordner mit Audiodateien zum Durchsuchen und Abspielen.
onboarding_stream_title = YouTube Music oder ein Server
onboarding_stream_desc = Verbinde YouTube Music (oder Jellyfin / Subsonic) zum Streamen und Herunterladen.
onboarding_open_settings = Einstellungen öffnen
onboarding_later = Später
discord_presence = Discord-Status
discord_test = Discord-Verbindung testen
discord_test_button = Test senden
connectivity = Konnektivität
discord_presence_paused = Pausierten Discord-Status anzeigen
reduce_animations = Animationen reduzieren
show_source_toggle = Quellenumschalter anzeigen
titlebar_mode = Titelleiste
titlebar_custom = Benutzerdefiniert (App-gezeichnet)
titlebar_system = Systemdekorationen
titlebar_off = Keine Titelleiste
volume = Lautstärke
local = Lokal
server = Server
ytdlp = Downloads

# UI Actions & Buttons
add_to_favorites = Zu Favoriten hinzufügen
remove_from_favorites = Aus Favoriten entfernen
add_to_queue = Zur Warteschlange hinzufügen
play_next = Als Nächstes abspielen
add_to_playlist = Zur Playlist hinzufügen
remove_from_playlist = Aus Playlist entfernen
share_musicbrainz = Teilen (MusicBrainz)
share_link_copied = MusicBrainz-Link kopiert
delete = Löschen
delete_song = Song löschen
delete_album = Album löschen
delete_playlist = Playlist löschen
delete_folder = Ordner löschen
rename_playlist = Playlist umbenennen
rename_folder = Ordner umbenennen
add_all_to_queue = Alle zur Warteschlange hinzufügen
add_all_to_playlist = Alle zur Playlist hinzufügen
select_all = Alle auswählen
select_all_tracks = Alle Tracks auswählen
remove_from_cache = Aus Cache entfernen
create = Erstellen
save = Speichern
enabled = AKTIVIERT
disabled = DEAKTIVIERT
play = Abspielen
shuffle = Zufall

# Navigation & Headers
tracks = Titel
albums = Alben
artists = Künstler
all_albums = Alle Alben
your_library = Deine Bibliothek
listening_logs = Hörprotokolle
browse_genres = Genres durchsuchen
top_artists = Top-Künstler
new_releases = Neuerscheinungen

# Navigation Buttons
back_to_albums = Zurück zu Alben
back_to_artists = Zurück zu Künstlern
back_to_playlists = Zurück zu Playlists
back_to_browse = Zurück zu Entdecken
back = Zurück
back_to_previous = Zurück zu
up_next = ALS NÄCHSTES
lyrics = SONGTEXT

# Player/Media
loading_lyrics = Songtext wird geladen...
lyrics_not_found = Songtext nicht gefunden
no_previous_songs = Keine vorherigen Titel
playlist_track_count = { $count } Titel
music_playlist_count = Musik • { $count } Titel
showcase_song_count = { $count } Titel

# Modal Titles
add_playlist = Playlist hinzufügen
create_new_playlist = Neue Playlist erstellen
playlist_name_placeholder = Playlist-Name
playlist_name_input = Playlist-Name
add_media_server = Medienserver hinzufügen
media_server = Medienserver
media_servers = Medienserver
saved_servers = Gespeicherte Server
switch_to_server = Wechseln
active_server = Aktiv
no_saved_servers = Keine gespeicherten Server
jellyfin = Jellyfin
subsonic = Subsonic
custom_manual = Benutzerdefiniert (manuelle API)
server_name_optional = Servername (optional)
server_url_placeholder = http://localhost:8096
change = Ändern
heart_track_to_add = Markiere einen Titel mit „Gefällt mir“, um ihn hier hinzuzufügen.
heart_track_to_add_server = Markiere einen Titel mit „Gefällt mir“, um ihn hier hinzuzufügen und mit deinem Server zu synchronisieren.

# Search & Placeholders
search_placeholder = Suche nach Künstlern, Alben oder Titeln...
no_results_found = Keine Ergebnisse für „{ $query }“ gefunden
listenbrainz_token_placeholder = Gib deinen ListenBrainz-Token ein
lastfm_api_key_placeholder = Gib deinen Last.fm-API-Schlüssel ein
lastfm_api_secret_placeholder = Gib dein Last.fm-API-Geheimnis ein

# Empty States
album_not_found = Album nicht gefunden
playlist_not_found = Playlist nicht gefunden
no_playlists_found = Du hast keine Playlists
no_playlists_yet = Du hast noch keine Playlists. Füge Songs aus deiner Bibliothek hinzu!
add_music_to_get_started = Füge Musik hinzu, um loszulegen

# Data Labels
title = Titel
time = Zeit
genre = Genre
plays = Wiedergaben
track_count = { $count } Titel
track_count_singular = 1 Titel
songs = Songs
min = Min.

# Error Messages
invalid_server_url = Die Server-URL ist ungültig
username_and_password_required = Bitte gib Benutzername und Passwort ein
login_failed = Anmeldung fehlgeschlagen: { $error }
error_server_not_configured = Server ist nicht eingerichtet oder Zugangsdaten fehlen
error_fetch_songs = Die Songs für das Album „{ $album_id }“ konnten nicht geladen werden: { $error }
unsupported_provider = { $service } ist eingerichtet, aber dieser Bereich wird noch nicht unterstützt
unsupported_provider_desc = Die anbieterabhängige Navigation für { $service } wird bald über die neue Server-Architektur verfügbar sein

# Additional Keys
no_genres_found = Keine Genres in deiner Bibliothek gefunden
jump_back_in = Weiterhören
by_artist = Von
by_artist_full = Von { $artist }
start_listening = Fang an zu hören
listen_now = Jetzt anhören
no_albums_found = Keine Alben in deiner Bibliothek gefunden
no_favorites = Noch keine Favoriten
no_tracks_found = Keine Titel gefunden
unknown_artist = Unbekannter Künstler
unknown_album = Unbekanntes Album
unknown_title = Unbekannter Titel
unknown_genre = Unbekannt
most_played_local_tracks = Deine meistgehörten lokalen Titel
no_tracks_in_library = Keine Titel in deiner Bibliothek gefunden
no_songs_here = Hier sind keine Songs
syncing_with_server = Synchronisiere mit dem Server...
yt_syncing_progress = Synchronisiere — { $count } Titel geladen
yt_synced_total = { $count } Lieblingstitel synchronisiert
refresh = Aktualisieren
most_played_tracks = Deine meistgehörten Titel
no_more_songs = Keine weiteren Songs in der Warteschlange
server_playlist = Server-Playlist
folder_playlists = Ordner-Playlists
folder_playlist = Ordner
featured_album = Vorgestelltes Album
minimize = Minimieren
maximize = Maximieren
close = Schließen
shuffle_on = Zufall: An
shuffle_off = Zufall: Aus
repeat_off = Wiederholen: Aus
repeat_queue = Wiederholen: Warteschlange
repeat_track = Wiederholen: Titel
rescan_library = Bibliothek neu scannen
refresh_music_library = Musikbibliothek aktualisieren
listenbrainz = ListenBrainz
lastfm = Last.fm
album_art_gradient = Albumcover-Verlauf
default_theme = Standard
gruvbox_material = Gruvbox Material
gruvbox_classic = Gruvbox Classic
gruvbox_dark_soft = Gruvbox Dark Soft
dracula = Dracula
nord = Nord
catppuccin_mocha = Catppuccin Mocha
ef_night = Ef Night
ayu_dark = Ayu Dark
ayu_mirage = Ayu Mirage
vague = Vague
one_dark_pro = One Dark Pro
osmium = Osmium
kanagawa_dragon = Kanagawa Dragon
everforest = Everforest
rosepine = Rosé Pine
default_light = Default Light
catppuccin_latte = Catppuccin Latte
rosepine_dawn = Rosé Pine Dawn
everforest_light = Everforest Light
ayu_light = Ayu Light
one_light = One Light
gruvbox_light_soft = Gruvbox Light Soft

# Theme Editor
new_theme = Neues Theme
theme_name = Theme-Name
my_custom_theme = Mein eigenes Theme
theme_group_dynamic = Dynamisch
theme_group_dark = Dunkel
theme_group_light = Hell
theme_group_custom = Benutzerdefiniert
colors = Farben
preview = Vorschau
save_theme = Theme speichern
track_title = Titel

# Color Labels
bg = Hintergrund
raised = Erhöhte Oberfläche
surface = Oberfläche
text = Text
text-muted = Gedämpfter Text
accent = Akzent
accent-soft = Sanfter Akzent
accent-alt = Alternativer Akzent
accent-deep = Dunkler Akzent
highlight = Hervorhebung
highlight-dark = Dunkle Hervorhebung
progress = Fortschritt
danger = Gefahr
equalizer = Equalizer
eq_preset = Preset
eq_preamp = Vorverstärkung
eq_preamp_desc = Headroom vor der EQ-Kurve
eq_graph_hint = Das Anpassen eines Bands schaltet das Preset auf Benutzerdefiniert.
eq_preset_flat = Flach
eq_preset_bass_boost = Bassanhebung
eq_preset_treble_boost = Höhenanhebung
eq_preset_vocal_boost = Stimmenanhebung
eq_preset_loudness = Loudness
eq_preset_custom = Benutzerdefiniert
move_to_folder = In Ordner verschieben
no_folders_yet = Noch keine Ordner.
folder_name = Ordnername...
new_folder = Neuer Ordner
remove_from_folder = Aus Ordner entfernen
back_behavior = Zurück-Button-Verhalten
back_behavior_rewind = ZURÜCKSPULEN → VORHERIGER
back_behavior_always_prev = IMMER VORHERIGER

ytdlp_title = Downloads
ytdlp_subtitle = Unterstützt von yt-dlp
ytdlp_options = Optionen
ytdlp_url_placeholder = YouTube-URL, Playlist, Kanal…
ytdlp_download = Herunterladen
ytdlp_output_dir_placeholder = Ausgabeverzeichnis (Standard: Musikordner)
ytdlp_pick_folder = Ordner auswählen
ytdlp_clear_history = Verlauf löschen
ytdlp_empty_state = Fügen Sie oben eine YouTube-URL ein, um zu beginnen
ytdlp_section_embed = In Datei einbetten
ytdlp_section_write = Separate Dateien schreiben
ytdlp_section_postprocess = Nachbearbeitung
ytdlp_section_behavior = Verhalten
ytdlp_embed_metadata = Metadaten einbetten
ytdlp_embed_thumbnail = Thumbnail einbetten
ytdlp_embed_chapters = Kapitel einbetten
ytdlp_embed_subtitles = Untertitel einbetten
ytdlp_embed_info_json = Info-JSON einbetten
ytdlp_write_thumbnail = Thumbnail speichern
ytdlp_write_description = Beschreibung speichern
ytdlp_write_info_json = Info-JSON speichern
ytdlp_write_subtitles = Untertitel speichern
ytdlp_write_auto_subtitles = Automatische Untertitel speichern
ytdlp_write_comments = Kommentare speichern
ytdlp_remove_sponsors = Sponsoren entfernen
ytdlp_mark_sponsors = Sponsoren als Kapitel markieren
ytdlp_split_chapters = Nach Kapiteln aufteilen
ytdlp_crop_thumbnails = Thumbnails mittig zuschneiden
ytdlp_single_video = Nur einzelnes Video
ytdlp_write_xattrs = xattrs schreiben
ytdlp_no_mtime = Datei-mtime nicht setzen
ytdlp_unlimited = unbegrenzt
ytdlp_none = Keine
ytdlp_js_runtimes_tooltip = Wenn YouTube mit JS- oder Captcha-Fehlern fehlschlägt, lege hier eine JS-Laufzeit fest, z. B. deno oder node.
ytdlp_format_best_audio = Bestes Audio
ytdlp_format_mp3 = MP3
ytdlp_format_flac = FLAC
ytdlp_format_wav = WAV
ytdlp_format_video = Video (MP4)
ytdlp_status_downloading_eta = { $percent }%  { $speed }  Noch { $eta }
ytdlp_status_downloading = { $percent }%
ytdlp_status_processing = Wird verarbeitet…
ytdlp_status_completed = Abgeschlossen
ytdlp_status_waiting = Warten…
ytdlp_error_not_found = yt-dlp im PATH nicht gefunden. Installieren: https://github.com/yt-dlp/yt-dlp
ytdlp_error_ffmpeg_not_found = ffmpeg im PATH nicht gefunden. Installieren Sie es, damit die Nachbearbeitung von yt-dlp funktioniert.
ytdlp_error_start = yt-dlp konnte nicht gestartet werden: { $error }
ytdlp_error_exit = yt-dlp beendet: { $status }
ytdlp_error_duplicate_active = Diese URL wird bereits heruntergeladen.
ytdlp_error_output_not_directory = Der Ausgabepfad ist kein Verzeichnis: { $path }
ytdlp_error_output_prepare = Ausgabeverzeichnis konnte nicht vorbereitet werden: { $error }
ytdlp_error_output_not_writable = Auf das Ausgabeverzeichnis kann nicht geschrieben werden: { $path }

channel_mode = Kanalmodus
channel_mode_stereo = Stereo
channel_mode_mono = Mono
channel_mode_left_only = Nur links
channel_mode_right_only = Nur rechts
channel_mode_swap_left_right = L/R tauschen

crossfade = Überblendung
crossfade_off = Aus

continue_listening = Weiterhören
music = Musik
made_for_you = Für dich gemacht
recently_added = Kürzlich hinzugefügt
customize_home = Startseite anpassen
done_editing = Fertig
reset_home = Zurücksetzen
hide_section = Ausblenden
show_section = Einblenden
move_up = Nach oben
move_down = Nach unten
home_section_hero = Hero
home_section_continue_listening = Weiterhören
home_section_listen_now = Jetzt hören
home_section_top_artists = Top-Künstler
home_section_new_releases = Neuerscheinungen
home_section_made_for_you = Für dich gemacht
home_section_recently_added = Kürzlich hinzugefügt
home_section_playlists = Playlists
layout_cards = Karten
layout_list = Liste
listen_now_layout = Layout
ui_style = UI-Stil
ui_normal = Normal
ui_modern = Vaxry
player_bar_position = Player-Leistenposition
position_bottom = Unten
position_top = Oben

metadata = Metadaten
artist_photo_source = Künstlerfoto-Quelle
artist_photo = Künstlerfoto
album_cover = Albumcover
auto_fetch_covers = Albumcover automatisch abrufen
prefer_local_lyrics = Lokale Liedtexte bevorzugen (.lrc / eingebettet, kein Online-Fallback)
cover_fetch_strategy = Cover-Abrufstrategie
musicbrainz_first = MusicBrainz zuerst
lastfm_first = Last.fm zuerst
musicbrainz_only = Nur MusicBrainz
lastfm_only = Nur Last.fm
remove_cover = Cover entfernen

# Radio Page
radio_filter_stations = Sender filtern…
radio_subtitle = Hören Sie Live-Internetradio-Streams
radio_no_stations_match = Keine Sender entsprechen Ihrer Suche
radio_station_col = Sender
radio_description_col = Beschreibung
radio_streams_col = Streams
radio_play = Abspielen
radio_listen_moe_desc = Anime und koreanische Popmusik, 24/7.
radio_j1_desc = Aktuelle Chart-Hits und klassische Lieder aus Japan.
radio_doujinstyle_desc = Doujin-Musik, Videospiel-Soundtracks und mehr.
radio_vocaloid_desc = Vocaloid-Hits aus Japan. Wir spielen alles!
radio_asiadreamradio_desc = Das Herz der J-Pop-Musik - J-Pop, J-Rock, J-HipHop, J-Jazz, Klassiker.
radio_live_stream = Live-Stream

auto_check_updates = Automatisch nach Updates suchen
update_available = Update verfügbar
update_banner_message = Kopuz { $version } ist verfügbar.
view_release = Release ansehen

volume_scroll_step = Lautstärke-Scrollschritt

# Radio Registries
radio_registry_url_placeholder = URL oder lokaler Pfad zu index.json
add_radio_registry = Radio-Verzeichnis hinzufügen
radio_default_registry = Standard-Verzeichnis


radio_registry_empty_path = Registry-Pfad darf nicht leer sein
radio_registry_exists = Registry existiert bereits
radio_registry_import_failed = Registry-Import fehlgeschlagen: { $error }
radio_registry_enable_failed = Registry konnte nicht aktiviert werden: { $error }
radio_registries_active = { $enabled_count }/{ $total } aktiv
radio_registries_empty = Keine Registries konfiguriert
offline_downloads = Offline-Downloads
download_quality = Download-Qualität
download_folder = Download-Ordner
remove_downloads = Downloads entfernen
download_all_offline = Alle für Offline-Wiedergabe herunterladen
premium_downloads = Premium-Downloads (256 kbps)
premium_downloads_hint = Meldet sich an, um 256-kbps-AAC zu laden. Nur mit YouTube Premium sinnvoll; kann unter hoher Last selten einen Bot-Check auslösen. Aus = anonym 128 kbps.
choose_folder = Auswählen…
reset_to_default = Zurücksetzen
saving = Speichern...
discover_more_loading = Lade weitere…
discover_failed = Entdecken konnte nicht geladen werden: { $error }
discover_requires_ytmusic = Entdecken ist nur verfügbar, wenn YouTube Music der aktive Server ist.
discover_show_all = Alle anzeigen
yt_anon_playlists = Melde dich bei YouTube Music an, um deine Bibliotheks-Playlists zu sehen.
yt_anon_favorites = Melde dich bei YouTube Music an, um deine markierten Songs zu sehen.

# Metadata Viewer / Editor
view_metadata = Metadaten anzeigen
edit_metadata = Metadaten bearbeiten
edit = Bearbeiten
track_number = Titelnr.
disc_number = CD-Nr.
duration = Dauer
sample_rate = Abtastrate
bitrate = Bitrate
musicbrainz_release = MusicBrainz-Veröffentlichung
musicbrainz_recording = MusicBrainz-Aufnahme
musicbrainz_track = MusicBrainz-Titel
path = Pfad
add_photo = Foto hinzufügen
change_photo = Foto ändern
remove_photo = Foto entfernen
metadata_edit_warning = Leere Felder entfernen das Tag. Wird direkt in die Datei geschrieben — kein Rückgängig.

# Spotify-Import
spotify_import_title = Von Spotify importieren
spotify_tab_url = Per URL
spotify_tab_account = Spotify-Konto
spotify_url_hint = Füge einen öffentlichen Spotify-Playlist- oder Album-Link ein — die komplette Titelliste wird importiert (ohne Login). Verbinde dein Konto für private Playlists und Lieblingssongs.
spotify_import_button = Zu YouTube Music importieren
spotify_fetching = Lade von Spotify…
spotify_matching = Suche auf YouTube Music — { $done } / { $total }
spotify_creating = Erstelle Playlist in deinem YouTube-Music-Konto…
spotify_adding = Füge Titel hinzu — { $done } / { $total }
spotify_done = { $matched } von { $total } Titeln importiert. Die Playlist ist jetzt in deiner YouTube-Music-Bibliothek.
spotify_unmatched = { $count } Titel konnten nicht zugeordnet werden
spotify_bad_url = Das sieht nicht nach einem Spotify-Playlist- oder Album-Link aus.
spotify_needs_yt_login = Melde dich zuerst bei YouTube Music an — die geklonte Playlist wird in deinem YT-Music-Konto erstellt.
spotify_connect_hint = Verbinde deine eigene Spotify-App (kostenlos), um private Playlists und Lieblingssongs zu importieren:
spotify_connect_step3 = Client-ID der App unten einfügen und verbinden.
spotify_connect_button = Spotify verbinden
spotify_waiting_browser = Warte auf Browser-Anmeldung…
spotify_connected = Spotify verbunden
spotify_disconnect = Trennen
spotify_load_playlists = Meine Playlists laden
spotify_liked_songs = Lieblingssongs
spotify_not_connected = Spotify ist nicht verbunden.
spotify_client_id_missing = Gib zuerst die Client-ID deiner Spotify-App ein.
spotify_import_from = Von Spotify importieren

# Sleep-Timer
sleep_timer = Sleep-Timer
sleep_timer_minutes = { $min } Minuten
sleep_timer_off = Ausschalten

# YT Music manual cookie auth
yt_auth_browser = Auto-Login (verwalteter Browser – empfohlen, bleibt angemeldet)
yt_auth_paste = Cookies einfügen (funktioniert unter Windows)
yt_auth_anonymous = Ohne Anmeldung fortfahren (anonym)
yt_anon_explainer = kopuz nutzt YouTube Music ohne Anmeldung. Stöbern, Suchen und Abspielen funktionieren — aber Lieblingsmusik, Bibliotheks-Playlists und Folgen/Liken sind deaktiviert.
yt_paste_step1 = Öffne ein PRIVATES / Inkognito-Fenster (Strg+Umschalt+N) und melde dich bei music.youtube.com an.
yt_paste_step2 = F12 → Netzwerk → neu laden → beliebige music.youtube.com-Anfrage anklicken → kompletten Cookie-Header kopieren.
yt_paste_step3 = Unten einfügen, dann das private Fenster SOFORT schließen — das friert die Session ein, die Cookies bleiben tagelang statt nur Minuten gültig.
yt_paste_firefox = Aus Firefox importieren
yt_paste_firefox_hint = Liest deine angemeldete YouTube-Sitzung direkt aus Firefox/LibreWolf.
yt_paste_invalid = YouTube hat diese Cookies abgelehnt — kopiere den vollständigen Cookie-Header aus einem angemeldeten music.youtube.com-Tab.
yt_manual_session_expired = Deine YouTube-Music-Sitzung ist abgelaufen. Füge frische Cookies ein: Einstellungen → Medienserver → YouTube Music neu hinzufügen.

# SoundCloud search
soundcloud_searching = Suche…
soundcloud_search_for = SoundCloud nach „{ $query }“ durchsuchen
soundcloud_download = Per yt-dlp herunterladen

# SoundCloud view + unified search
soundcloud_hint = Oben tippen, um SoundCloud zu durchsuchen.
soundcloud_needs_ytdlp = Die SoundCloud-Suche benötigt ein installiertes yt-dlp.
no_results = Keine Ergebnisse

# Toast
added_to_playlist_toast = Zu „{ $name }“ hinzugefügt

# YT auto-refresh
yt_auto_refresh_label = Automatisch eingeloggt bleiben — Cookies beim Start aus einem eingeloggten Browser holen (in Firefox bei YouTube angemeldet bleiben; kein F12 mehr nötig).

# YT OAuth device-code sign-in
yt_auth_oauth = Mit Google anmelden (empfohlen — bleibt angemeldet, kein Browser nötig)
yt_oauth_explainer = Einmal im Browser anmelden. kopuz hält dich danach automatisch angemeldet — ohne offenen Browser und ohne Cookies neu einzufügen. Ideal für ressourcenschonende Wiedergabe im Hintergrund.
yt_oauth_button = Mit Google anmelden
yt_oauth_enter_code = Öffne die Seite und gib diesen Code ein:
yt_oauth_waiting = Warte auf deine Bestätigung…
yt_oauth_done = Angemeldet! Klicke auf Speichern zum Abschließen.
yt_oauth_timeout = Der Anmelde-Code ist abgelaufen — bitte erneut starten.
yt_oauth_required = Bitte zuerst mit Google anmelden.

download_song = Download

# Delete playlist
delete_playlist_confirm = Permanently delete “{ $name }”? This cannot be undone.

# YT OAuth client setup
yt_oauth_setup_title = Einmalige Einrichtung: dein eigener Google-OAuth-Client
yt_oauth_setup_step1 = Google-Cloud-Projekt anlegen und „YouTube Data API v3“ aktivieren.
yt_oauth_setup_step2 = Zustimmungsbildschirm (Extern) einrichten und App VERÖFFENTLICHEN, damit der Login nicht wöchentlich abläuft.
yt_oauth_setup_step3 = OAuth-Client vom Typ „Fernseher und Geräte mit begrenzter Eingabe“ erstellen.
yt_oauth_setup_step4 = Dessen Client-ID und Clientschlüssel unten einfügen — nur einmal nötig.
yt_oauth_need_creds = Erst oben Client-ID und Client-Secret eintragen.
yt_oauth_full_guide = Vollständige Schritt-für-Schritt-Anleitung öffnen

# YT managed-browser login modal
yt_login_title = Bei YouTube Music anmelden
yt_login_body = Es hat sich ein Browserfenster geöffnet. Melde dich dort bei deinem YouTube-Konto an und klicke dann auf „Fertig“ — Kopuz übernimmt die Sitzung und hält dich ab jetzt automatisch angemeldet.
yt_login_done = Fertig – ich bin angemeldet
yt_login_not_done = Noch nicht angemeldet — bitte im Browserfenster fertig anmelden und erneut auf „Fertig“ klicken.

autoradio_setting = Autoplay radio when the queue ends

spotify_403_follow = Spotify only lets the app import playlists you OWN or FOLLOW. Open this playlist in Spotify, tap Follow (the heart), then import again — or use “Your playlists” after following. (A Spotify restriction for personal apps, not a kopuz bug.)
spotify_reconnect_follow = Could not follow the playlist — your Spotify connection is missing the follow permission. Click Disconnect, then Connect Spotify again (re-authorize in the browser), and retry. Kopuz will then auto-follow and import the whole playlist.
spotify_follow_no_help = Followed the playlist, but Spotify still blocks reading its full track list for personal apps. This looks like a hard Spotify limit — only playlists you own import in full.
