...
onboarding_title = Welcome to Kopuz
onboarding_subtitle = Two ways to fill it with music:
onboarding_local_title = Local music folder
onboarding_local_desc = Point Kopuz at a folder of audio files to browse and play them.
onboarding_stream_title = YouTube Music or a server
onboarding_stream_desc = Connect YouTube Music (or Jellyfin / Subsonic) to stream and download.
onboarding_open_settings = Open Settings
onboarding_later = Maybe later
discord_presence = Discord Presence
discord_test = Test Discord connection
discord_test_button = Send test
...
yt_syncing_progress = Syncing — { $count } tracks loaded so far
yt_synced_total = { $count } liked tracks synced
refresh = Refresh
discover = Discover
...
yt_anon_playlists = Sign in to YouTube Music to see your library playlists.
yt_anon_favorites = Sign in to YouTube Music to see your liked songs.
...
spotify_url_hint = Paste a public Spotify playlist or album link — the full track list is imported (no login needed). Connect your account for private playlists and Liked Songs.
...
spotify_search_hint = Type above to search Spotify.
spotify_searching = Searching Spotify…
spotify_search_needs_account = Spotify search needs your connected Spotify account. Open Playlists → Import from Spotify and connect your (free) Spotify app there.
spotify_finding_match = Finding “{ $title }” on YouTube Music…
spotify_no_match = No YouTube Music match for “{ $title }”.
spotify_playback_note = Spotify audio is copy-protected — kopuz plays the matching track from YouTube Music.
...
yt_paste_step1 = Open a PRIVATE / Incognito window (Ctrl+Shift+N) and sign in at music.youtube.com.
yt_paste_step2 = Press F12 → Network → reload → click any music.youtube.com request → copy the full Cookie header.
yt_paste_step3 = Paste it below, then CLOSE the private window right away — this freezes the session so the cookies stay valid for days instead of minutes.
...
search_elsewhere = Nothing on { $source } — other sources have:
