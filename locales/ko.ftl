...
playlists = 재생목록
playlist = 재생목록
playlist_create_failed = 재생목록을 만들지 못했습니다: { $error }
playlist_create_unsupported = 이 곡은 활성 서버에 저장할 수 없습니다.
album = 앨범
artist = 아티스트
home = 홈
search = 검색
...
add_server = 서버 추가
connect_to_lastfm = Last.fm에 연결
lastfm_connected = Last.fm 연결됨
...
yt_syncing_progress = 동기화 중 — 지금까지 { $count }개의 트랙을 불러왔습니다
yt_synced_total = { $count }개의 좋아하는 트랙이 동기화됨
refresh = 새로고침
...
radio_asiadreamradio_desc = 제이팝 음악의 중심 - 제이팝, 제이락, 제이힙합, 제이재즈, 클래식.
...
sample_rate = 샘플 레이트
bitrate = 비트레이트
musicbrainz_release = MusicBrainz 릴리스
musicbrainz_recording = MusicBrainz 레코딩
musicbrainz_track = MusicBrainz 트랙
path = 경로
add_photo = 사진 추가
change_photo = 사진 변경
remove_photo = 사진 제거
...
choose_folder = 선택…
discord_test = Discord 연결 테스트
discord_test_button = 테스트 보내기
download_all_offline = 오프라인 재생용으로 전체 다운로드
download_folder = 다운로드 폴더
onboarding_title = Kopuz에 오신 것을 환영합니다
onboarding_subtitle = 음악으로 채우는 두 가지 방법:
onboarding_local_title = 로컬 음악 폴더
onboarding_local_desc = 오디오 파일이 있는 폴더를 Kopuz에 지정하면 둘러보고 재생할 수 있습니다.
onboarding_stream_title = YouTube Music 또는 서버
onboarding_stream_desc = YouTube Music(또는 Jellyfin / Subsonic)을 연결하면 재생하고 다운로드할 수 있습니다.
onboarding_open_settings = 설정 열기
onboarding_later = 나중에
premium_downloads = 프리미엄 다운로드 (256 kbps)
premium_downloads_hint = 로그인하여 256 kbps AAC를 받아옵니다. YouTube Premium 계정에서만 효과가 있으며, 부하가 클 때 드물게 봇 확인이 뜰 수 있습니다. 끄면 익명 128 kbps입니다.
remove_downloads = 다운로드 제거
reset_to_default = 기본값으로 재설정
search_elsewhere = { $source }에 없음 — 다른 소스에는 있습니다:
