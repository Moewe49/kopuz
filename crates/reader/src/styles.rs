//! The 400 style names the model's activation head is trained on.
//!
//! Generated from the model's own metadata — the `classes` list in
//! `discogs-effnet-bsdynamic-1.json`. Names only: the weights stay a runtime
//! download for licence reasons, see `embed::model`. Having the names in the
//! binary is what lets a mix be called "Shoegaze" rather than after two of
//! the artists that happen to be in it.
//!
//! Order matters — index `i` here is dimension `i` of the activation vector.

/// `Genre---Subgenre`, in activation order.
pub const STYLES: [&str; 400] = [
    "Blues---Boogie Woogie",
    "Blues---Chicago Blues",
    "Blues---Country Blues",
    "Blues---Delta Blues",
...
/// The subgenre half — what a listener would call it.
pub fn subgenre(index: usize) -> &'static str {
    STYLES
        .get(index)
...
/// The broad genre half.
pub fn genre(index: usize) -> &'static str {
    STYLES
        .get(index)
...
mod tests {
    ...
    /// Index i must be dimension i of the activation vector, or every mix
    /// gets named after the wrong style.
    #[test]
    fn the_list_matches_the_activation_width() {
        assert_eq!(STYLES.len(), 400);
        assert!(
            STYLES.iter().all(|s| s.contains("---")),
            "a name is missing its genre"
        );
    }

    #[test]
    fn a_name_splits_into_genre_and_subgenre() {
        let i = STYLES
            .iter()
            .position(|s| *s == "Rock---Shoegaze")
            .expect("Shoegaze");
        assert_eq!(genre(i), "Rock");
        assert_eq!(subgenre(i), "Shoegaze");
        assert_eq!(subgenre(9999), "");
    }
}
