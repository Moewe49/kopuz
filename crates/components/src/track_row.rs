    ...
    if is_modern {
        return rsx! {
            div {
                ...
                // On a phone there is no hover to reveal the menu, so the dots
                // stay visible — otherwise every per-track action (remove from
                // playlist, add to playlist, queue…) is simply unreachable on
                // mobile, which is why "can't remove songs from playlists".
                div { class: if cfg!(target_os = "android") {
            ...
            div { class: "flex items-center justify-end",
                if !is_selection_mode {
                    DotsMenu {
                        ...
                        on_open: move |_| on_click_menu.call(()),
                        on_close: move |_| on_close_menu.call(()),
                        button_class: if cfg!(target_os = "android") {
                            String::new()
                        } else {
                            "opacity-0 group-hover:opacity-100 focus:opacity-100".to_string()
                        },
                        anchor: "right".to_string(),
                        on_action: move |idx: usize| {
                            if let Some(play_next_idx) = play_next_idx
