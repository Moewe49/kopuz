...
/// Extra Gradle dependencies appended to the dx-scaffolded app `build.gradle(.kts)`
/// (mirrors android-src/patch_gradle.py). androidx.webkit is already transitive via
/// the wry template; media3 (ExoPlayer + session) is NOT — PlaybackService needs it
/// to run playback in a MediaSessionService that survives backgrounding. Gradle
/// merges multiple `dependencies {}` blocks, so appending is safe for Groovy or KTS.
const GRADLE_DEPS: &[&str] = &[
    "androidx.media3:media3-exoplayer:1.4.1",
    "androidx.media3:media3-session:1.4.1",
    // HLS: some SoundCloud streams have no progressive mp3, only an HLS
    // playlist — ExoPlayer needs this module to play those.
    "androidx.media3:media3-exoplayer-hls:1.4.1",
];
const GRADLE_MARKER: &str = "// AUTO-GENERATED by kopuz/build.rs — media3 deps";
...
/// Pre-Android-12 backup rules (`android:fullBackupContent`).
const BACKUP_RULES: &str = r#"<?xml version="1.0" encoding="utf-8"?>
...
/// Android 12+ backup + device-to-device transfer rules
/// (`android:dataExtractionRules`).
const DATA_EXTRACTION_RULES: &str = r#"<?xml version="1.0" encoding="utf-8"?>
<!-- AUTO-GENERATED by kopuz/build.rs -->
...
fn main() {
    ...
    // 4b-ii. Backup rules, so the session survives a reinstall / device swap.
    let res_xml = main_dir.join("res").join("xml");
    copy_str(&res_xml.join("kopuz_backup_rules.xml"), BACKUP_RULES);
    copy_str(
        &res_xml.join("kopuz_data_extraction.xml"),
        DATA_EXTRACTION_RULES,
    );

    // 4c. Inject the media3 (ExoPlayer) Gradle deps PlaybackService needs. The app
    //     module dir is main_dir/../.. (app/src/main -> app).
    if let Some(module_dir) = main_dir.parent().and_then(|p| p.parent()) {
        patch_gradle_deps(module_dir);
    }
...
/// Append the media3 dependencies to the app module's `build.gradle(.kts)` (idempotent,
/// mirrors android-src/patch_gradle.py). dx regenerates the project each build, so this
/// re-runs every time; the marker guard makes a re-run a no-op if dx ever keeps the file.
fn patch_gradle_deps(module_dir: &Path) {
    for name in ["build.gradle.kts", "build.gradle"] {
        ...
        if src.contains(GRADLE_MARKER) {
            return;
        }
        let kts = name.ends_with(".kts");
        let lines: Vec<String> = GRADLE_DEPS
            .iter()
            .map(|d| {
                if kts {
                    format!("    implementation(\"{d}\")")
                } else {
                    format!("    implementation '{d}'")
                }
            })
            .collect();
        let block = format!("\n\n{GRADLE_MARKER}\ndependencies {{\n{}\n}}\n", lines.join("\n"));
        if let Err(e) = fs::write(&path, format!("{src}{block}")) {
            warn(&format!("cannot patch {}: {e}", path.display()));
        }
        return;
    }
    warn(&format!(
        "no app build.gradle(.kts) under {}",
        module_dir.display()
    ));
}
...
fn patch_manifest(path: &Path) {
    ...
    // Back up the app's own files (config.json with the server + YouTube cookies,
    // playlists.json, the library index) so a reinstall or a new phone doesn't
    // dump the user back into onboarding. The in-app updater installs in place
    // and keeps data by itself; this is the safety net for the cases it can't
    // cover — a manual uninstall, a device swap, or moving off a build that was
    // signed with a different key. Caches are excluded: re-downloadable, and a
    // multi-GB offline cache would blow the backup quota.
